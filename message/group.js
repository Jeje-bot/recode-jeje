...
module.exports = welcome = async (ikyy, anu) => {
		...
		try {
            ...
            buff = await getBuffer(`http://hadi-api.herokuapp.com/api/card/welcome2?nama=${encodeURI(anu_user)}&descriminator=${memeg}&memcount=${memeg}&gcname=${encodeURI(mdata.subject)}&gcicon=${pp_grup}&pp=${pp_user}&bg=http://bit.ly/2ZSeWsV`)
        buttons = [
        ...
        buttonsMessage = {
          ...
          footerText: "Developer by Muhamad Nahara",
      ...
      if (anu.action == "remove" && !mem.includes(ikyy.user.jid)) {
        ...
        buff = await getBuffer(`http://hadi-api.herokuapp.com/api/card/goodbye2?nama=${encodeURI(anu_user)}&descriminator=${memeg}&memcount=${memeg}&gcname=${encodeURI(mdata.subject)}&gcicon=${pp_grup}&pp=${pp_user}&bg=http://bit.ly/2ZSeWsV`)
