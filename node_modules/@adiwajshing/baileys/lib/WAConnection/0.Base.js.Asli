        ...
        while (triesLeft >= 0) {
            ...
            try {
                ...
                if (expect200 && response.status && Math.floor(+response.status / 100) !== 2) {
                    ...
                    throw new Constants_1.BaileysError(`Unexpected status in '${json[0] || 'query'}': ${http_1.STATUS_CODES[response.status]}(${response.status})`, { query: json, message, status: responsestatus });
                }
